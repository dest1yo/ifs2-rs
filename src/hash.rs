...
/// Utility to hash data with crc32 algo.
pub trait HashCRC32 {
    /// Creates a crc32 checksum for this data.
    fn hash_crc32(&self, initial: u32) -> u32;
}

impl HashCRC32 for &str {
    fn hash_crc32(&self, initial: u32) -> u32 {
        hash_crc32_string_int(self.as_bytes(), initial)
    }
}

impl HashCRC32 for String {
    fn hash_crc32(&self, initial: u32) -> u32 {
        hash_crc32_string_int(self.as_bytes(), initial)
    }
}

impl HashCRC32 for &[u8] {
    fn hash_crc32(&self, initial: u32) -> u32 {
        hash_crc32_string_int(self, initial)
    }
}

fn hash_crc32_string_int(input: &[u8], initial: u32) -> u32 {
    let mut crc = !initial;
...
// Note: Copied from wow-mpq lib for pub use
...
fn test_crc32_algorithm() {
    ...
    let test_cases: Vec<(&[u8], u32)> = vec![
        (b"".as_ref(), 0x00000000),
        (b"a".as_ref(), 0x9FB08ED5),
        (b"abc".as_ref(), 0x3762FF9B),
        (b"Hello, World!".as_ref(), 0xAB66A3AE),
        (
            b"The quick brown fox jumps over the lazy dog".as_ref(),
            0x5234F702,
        ),
        (b"sco_l115dragonboat_col.iwi".as_ref(), 0x39076311),
    ];

    for (data, expected) in test_cases {
        let crc = hash_crc32_string_int(data, data.len() as u32);
        assert_eq!(
            crc, expected,
